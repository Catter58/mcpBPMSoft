...
- перевод «Город = Москва» в фильтр по справочнику — либо `City/Name eq 'Москва'`, либо `CityId = <UUID>` на записи;
...
В коробке **32 операционных инструмента (+ `bpm_init` при включённом env-creds opt-in), 6 prompts, 4 ресурса** — от низкоуровневого CRUD до готовых сценариев «зарегистрировать контакт + контрагента» и «найти всё про Иванова».
...
- что русские названия полей доступны только через системную таблицу `SysEntitySchemaColumn`, а на части стендов её нет вовсе;
...
| Инструмент           | Зачем                                                                                                                                                                                                                                                     |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bpm_get_records`    | Получить записи коллекции с фильтром/select/expand/order/top/skip; **без `select` — только `Id` + колонка отображения**, все колонки — по `select='*'`; **safe-pagination** + **token-aware форматы** (compact/full/markdown); поддерживает opaque cursor |
| `bpm_get_record`     | Одна запись по UUID (та же проекция по умолчанию)                                                                                                                                                                                                         |
| `bpm_count_records`  | Количество записей по фильтру                                                                                                                                                                                                                             |
| `bpm_whoami`         | Кто выполняет вызов (SysAdminUnit + Contact) и текущее время в его поясе                                                                                                                                                                                  |
...
| Инструмент              | Зачем                                                                                                                                         |
| ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `bpm_get_collections`   | Список доступных EntitySet (limit по умолчанию 100, в ответе total/has_more)                                                                  |
...
| Инструмент         | Зачем                            |
| ------------------ | -------------------------------- |
| `bpm_batch_create` | Создать N записей за один вызов  |
| `bpm_batch_update` | Обновить N записей за один вызов |
| `bpm_batch_delete` | Удалить N записей за один вызов  |

Все три поддерживают `continue_on_error`. Способ отправки сервер выбирает сам: при первом пакетном
вызове он проверяет `$batch` безвредным GET; если инстанс его не поддерживает (или это OData v3),
запросы уходят по одному, последовательно. В ответе поле `mode`: `batch` или `single`.
...
**Почему в ответе только `Id` и `Name`, а не все колонки?**
Это защита контекста: без `select` сервер сам ставит `$select=Id,<колонка отображения>` (`Name`/`Title`/`Subject`/…). Нужны конкретные поля — перечислите их в `select`; нужна вся запись — передайте `select='*'`. То же и у `bpm_get_record`/`bpm_search_records`.

**В выдаче `CityId` — как понять, что это за город, без второго запроса?**
Никак не надо: рядом с каждой lookup-колонкой сервер кладёт её имя — `CityId` + `CityName`. Имена добираются одним запросом на справочник (не на запись) и настоящие колонки не затираются. Отключается параметром `resolve_lookups: false`.

**Модель просит «поставь мне задачу на сегодня». Что она должна вызвать?**
`bpm_whoami` — он возвращает и текущего пользователя с его контактом, и реальное время в его поясе. Личность вычисляет сам BPMSoft макросом DataService в сессии вызывающего, поэтому при per-request авторизации ответ соответствует владельцу cookie/BPMCSRF, а не тому, кто залогинился последним. Дальше `contact_id` подставляется в `OwnerId`/`AuthorId`, а «сегодня» в критериях считается оператором `сегодня`.

**Как задать «за сегодня» или «в этом месяце»?**
Оператором, а не датами: `{"field": "Дата создания", "op": "сегодня"}`. Есть `сегодня`, `вчера`, `завтра`, `на этой неделе`, `на прошлой неделе`, `в этом месяце`, `в прошлом месяце`, `в этом квартале`, `в этом году` (и английские синонимы). Границы считаются по календарю **часового пояса пользователя** и компилируются в полуинтервал `ge …T21:00:00Z and lt …T21:00:00Z` — для Москвы это ровно местные сутки, а не UTC-шные. Пояс берётся из `BPMSOFT_TIMEZONE`, иначе — пояс процесса.

**Можно посмотреть запрос, не выполняя его?**
`dry_run: true` у `bpm_get_records` и `bpm_search_records` вернёт собранный URL, `$filter`, `$select` и `$expand`, ничего не запрашивая. Удобно проверить, как разрешились имена полей и во что превратились критерии.

**Модель написала «Контакт» вместо «Contact».**
Сработает. Имя коллекции резолвится сервером по схеме: русская подпись объекта берётся из `SysSchema.Caption`, регистр не важен, опечатка возвращает `not_found` со списком похожих имён (`Contactt` → `Contact, ContactTag, Contract, …`), а не голый 404.

**Надо отфильтровать по справочному полю. Сначала доставать UUID?**
Нет. `bpm_search_records` для lookup-колонки со строковым значением сам сравнивает с именем связанной записи: `{"field": "Тип", "op": "равно", "value": "Сотрудник"}` компилируется в `Type/Name eq 'Сотрудник'`. UUID в значении по-прежнему сравнивается с самой FK-колонкой — так дешевле для сервера, а `пусто`/`не пусто` всегда проверяются на FK.

**Поиск по подстроке молча вернул пусто / упал сетевой ошибкой.**
Так вело себя до фикса: часть стендов не переваривает `tolower()` в `$filter` и вместо честного 400 отдаёт 200 и обрывает тело ответа. Теперь обрыв потока распознаётся как отказ от конструкции: сервер запоминает это на весь процесс, пересобирает фильтр без `tolower()` и повторяет запрос, а в `warnings` пишет, что поиск выполнен с учётом регистра.

**Как назвать поле — «Город» или «City»?**
Любым. Сервер сам тянет схему коллекции и сопоставляет колонку: сначала имя, затем caption из `SysSchema`/`SysEntitySchemaColumn`, затем встроенный словарь русских подписей типовых колонок Creatio. Последний нужен потому, что на части стендов представления с caption'ами колонок недоступны (404) и подписей нет вовсе. Имена колонок в `select` проверяются там же — `select: "Название,Город"` уходит в BPMSoft как `Id,Name,CityId`, а опечатка возвращает ошибку с подсказками вместо 400 от сервера.

**Lookup-значения — UUID или текст?**
Текст. Сервер сам резолвит «Москва» → `<UUID>` через `bpm_lookup_value`. Если найдено несколько кандидатов — вернёт список и попросит уточнить. Работает и на запись (`bpm_create_record`, `bpm_update_record`, `bpm_batch_*`, `bpm_update_by_filter`), причём ключ можно давать русской подписью («Город»). Пустая строка в lookup-поле очищает связь.
